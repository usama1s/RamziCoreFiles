<?php
class wtwdownloads {
	/* $wtwdownloads class for admin database functions for 3d communities */
	...
	public function addDownloadQueue($zwebid, $zwebtype) {
		...
		try {
			...
			$wtwconnect->serror("core-functions-class_wtwdownloads.php-addDownloadQueue=".$e->getMessage());
			$zresponse = array(
				'success'=>'0',
				'serror'=>$e->getMessage()
			);
	...
	public function updateDownloadsQueue($zdownloadid, $zwebid, $zwebtype, $zprocess) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-updateDownloadsQueue=".$e->getMessage());
			$zresponse = array(
				'serror'=>$e->getMessage()
			);
		}
	...
	public function downloadWeb($zwebid, $znewwebid, $zwebtype, $zusertoken, $zdownloadparentwebid, $zdownloadparentwebtype, $zcommunityid, $zbuildingpositionx = 0, $zbuildingpositiony = 0, $zbuildingpositionz = 0, $zbuildingscalingx = 1, $zbuildingscalingy = 1, $zbuildingscalingz = 1, $zbuildingrotationx = 0, $zbuildingrotationy = 0, $zbuildingrotationz = 0) {
		/* this process downloads 3D Web and dependent objects from https://d3net.bixma.com (roomz repository)*/
		...
		try {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				$zurl = "https://d3net.bixma.com/connect/sharedownload.php?webid=".$zwebid."&webtype=".$zwebtype."&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadWeb=".$e->getMessage());
			$zresponse = array(
				'webid' => $zwebid,
				'newwebid' => $znewwebid,
				'webtype' => $zwebtype,
	...
	public function downloadWebProgress($zwebid, $znewwebid, $zwebtype, $zusertoken, $zdownloadparentwebid, $zdownloadparentwebtype, $zcommunityid, $zbuildingpositionx = 0, $zbuildingpositiony = 0, $zbuildingpositionz = 0, $zbuildingscalingx = 1, $zbuildingscalingy = 1, $zbuildingscalingz = 1, $zbuildingrotationx = 0, $zbuildingrotationy = 0, $zbuildingrotationz = 0) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadWebProgress=".$e->getMessage());
	...
	public function downloadUsers($zrequestusers, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUsers=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadUploads($zrequestuploads, $znewfolder, $znewurl, $zusertoken) {
		...
		try {
			...
			foreach ($zrequestuploads as $zupload) {
				...
				if (!empty($zupload->filepath)) {
					try {
						...
						$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUploads-getfile=".$e->getMessage());
						$zresponse = array(
							'serror' => $e->getMessage()
						);
					}	
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUploads=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadMainWeb($zrequest, $zwebtype, $znewwebid, $zdownloadparentwebtype, $zdownloadparentwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadMainWeb=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadActionZonesAvatarAnimations($zrequestavataranimations, $znewfolder, $zusertoken) {
		...
		try {
				...
				if (!empty($zavataranimation->objectfolder) && !empty($zavataranimation->objectfile)) {
					...
					if ($zfileext == 'babylon') {
						try {
							...
							$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadActionZonesAvatarAnimations-getfile-babylon=".$e->getMessage());
							$zresponse = array(
								'serror' => $e->getMessage()
							);
						}	
						try {
							...
							$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadActionZonesAvatarAnimations-getfile-manifest=".$e->getMessage());
							$zresponse = array(
								'serror' => $e->getMessage()
							);
						}	
						...
						if (!empty($zavataranimation->animationicon)) {
							try {
								...
								$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadActionZonesAvatarAnimations-getfile-icon=".$e->getMessage());
								$zresponse = array(
									'serror' => $e->getMessage()
								);
							}	
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadActionZonesAvatarAnimations=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadActionZones($zrequestactionzones, $znewcommunityid, $znewbuildingid, $znewthingid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadActionZones=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadScripts($zrequestscripts, $znewfolder, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadScripts=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadConnectingGrids($zrequestconnectinggrids, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadConnectingGrids=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadContentRatings($zrequestcontentratings, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadContentRatings=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadPluginsRequired($zrequestpluginsrequired, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadPluginsRequired=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadUploadObjects($zrequestuploadobjects, $znewfolder, $znewurl, $zusertoken) {
		...
		try {
				...
				if ($wtwhandlers->hasValue($zuploadobject->uploadobjectid)) {
					...
					foreach($zuploadobject->objectfiles as $zfile) {
						try {
							...
							$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUploadObjects-getfiles=".$e->getMessage());
							$zresponse = array(
								'serror' => $e->getMessage()
							);
						}	
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUploadObjects=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadMolds($zrequestmolds, $zwebtype, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadMolds=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadAddFirstBuilding($znewwebid, $zwebtype, $zcommunityid, $zbuildingpositionx, $zbuildingpositiony, $zbuildingpositionz, $zbuildingscalingx, $zbuildingscalingy, $zbuildingscalingz, $zbuildingrotationx, $zbuildingrotationy, $zbuildingrotationz, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAddFirstBuilding=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadChildConnectingGrids($zrequestchildconnectinggrids, $znewwebid, $zwebtype, $zusertoken, $isprogress) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadChildConnectingGrids=".$e->getMessage());
			$zresponse[0] = array(
				'childtemplatename' => '',
				'webid' => '',
				'newwebid' => '',
	...
	public function downloadUpdateWeb($zwebid, $zupdatewebid, $zwebtype, $zusertoken) {
		/* this process downloads updated version of 3D Web from https://d3net.bixma.com (roomz repository)*/
...
//				$zwebid = $wtwhandlers->getNewKey($zwebtypes, $zwebtype."id", $zwebid);
				...
				/* fetch the 3D Web data structure for the repository (all of the associated records) */
				$zurl = "https://d3net.bixma.com/connect/sharedownload.php?webid=".$zupdatewebid."&webtype=".$zwebtype."&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
				...
				foreach ($zrequest->uploads as $zupload) {
					...
					if (!empty($zupload->filepath)) {
						try {
							...
							$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfile=".$e->getMessage());
						}	
					}
					...
					if (!empty($zavataranimation->objectfolder) && !empty($zavataranimation->objectfile)) {
						...
						if ($zfileext == 'babylon') {
							try {
								...
								$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfile-babylon=".$e->getMessage());
							}	
							try {
								file_put_contents($znewobjectfolder."/".$zavataranimation->objectfile.".manifest", fopen($zavataranimation->objectfolder.$zavataranimation->objectfile.".manifest", 'r'));
								umask(0);
								...
								$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfile-manifest=".$e->getMessage());
							}	
							...
							if (!empty($zavataranimation->animationicon)) {
								try {
									...
									$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfile-icon=".$e->getMessage());
								}	
							}
						}
					}
					...
					if (!empty($zscript->scriptpath)) {
						...
						if ($zfileext == 'js') {
							try {
								...
								$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfile-js=".$e->getMessage());
							}	
						}
					}
					...
					if ($wtwhandlers->hasValue($zuploadobject->uploadobjectid)) {
						...
						foreach($zuploadobject->objectfiles as $zfile) {
							try {
								...
								$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb-getfiles=".$e->getMessage());
							}	
						}
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateWeb=".$e->getMessage());
		}
		return $zwebid;
	}
	...
	public function downloadAvatar($zwebid, $znewwebid, $zusertoken) {
		...
		try {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				/* fetch the 3D Web data structure for the repository (all of the associated records) */
				$zurl = "https://d3net.bixma.com/connect/sharedownload.php?webid=".$zwebid."&webtype=avatar&userid=".$zuserid."&serverinstanceid=".$wtwhandlers->serverinstanceid."&domainurl=".$wtwhandlers->domainurl;
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatar=".$e->getMessage());
			$zresponse = array(
				'serror'=> $e->getMessage(),
				'webid'=> $zwebid,
				'newwebid'=> $znewwebid,
	...
	public function downloadAvatarProgress($zwebid, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatarProgress=".$e->getMessage());
			$zresponse = array(
				'serror'=> '',
				'webid' => $zwebid,
				'newwebid' => $znewwebid,
	...
	public function downloadMainAvatar($zrequest, $znewwebid, $zobjectfolder, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadMainAvatar=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
		...
		try {
			...
			foreach ($zrequestfiles as $zfile) {
				if (strpos($zfile->file,'/') !== false) {
					...
					$wtwhandlers->getFilefromURL('https://d3net.bixma.com/content/uploads/avatars/'.$zwebid.'/'.$zfolder.'/'.$zfilename, wtw_rootpath.$zobjectfolder.$zfolder.'/', $zfilename);
				} else {
					$wtwhandlers->getFilefromURL('https://d3net.bixma.com/content/uploads/avatars/'.$zwebid.'/'.$zfile->file, wtw_rootpath.$zobjectfolder, $zfile->file);
				}
			}
		} catch (Exception $e) {
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatarFiles=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadAvatarGroup($zavatargroup, $znewcreateuserid, $znewupdateuserid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatarGroup=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadAvatarParts($zrequestavatarparts, $znewwebid, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatarParts=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadAvatarAnimations($zrequestavataranimationdefs, $znewwebid, $zobjectfolder, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadAvatarAnimations=".$e->getMessage());
			$zresponse = array(
				'serror' => $e->getMessage()
			);
		}
	...
	public function downloadUpdateAvatar($zwebid, $zupdatewebid, $zwebtype, $zusertoken) {
		...
		try {
			...
			if ($wtwhandlers->hasValue($zuserid)) {
				...
				foreach ($zrequest->files as $zfile) {
					if (strpos($zfile->file,'/') !== false) {
						...
						$wtwhandlers->getFilefromURL('https://d3net.bixma.com/content/uploads/avatars/'.$zupdatewebid.'/'.$zfolder.'/'.$zfilename, wtw_rootpath.$zobjectfolder.$zfolder.'/', $zfilename);
					} else {
						$wtwhandlers->getFilefromURL('https://d3net.bixma.com/content/uploads/avatars/'.$zupdatewebid.'/'.$zfile->file, wtw_rootpath.$zobjectfolder, $zfile->file);
					}
				}
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateAvatar=".$e->getMessage());
			$zresponse = array(
				'serror'=> $e->getMessage(),
				'webid'=> $zwebid,
				'webtype'=> $zwebtype
	...
	public function downloadUpdateUserAvatar($zuseravatarid, $zinstanceid, $zwebid, $zupdatewebid, $zwebtype, $zusertoken) {
		...
		try {
			...
			$wtwhandlers->serror("core-functions-class_wtwdownloads.php-downloadUpdateUserAvatar=".$e->getMessage());
			$zresponse = array(
				'serror'=> $e->getMessage(),
				'useravatarid'=> $zuseravatarid,
				'webid'=> $zwebid,
	...
	function wtwdownloads() {
		return wtwdownloads::instance();
	}

	/* Global for backwards compatibility. */
	$GLOBALS['wtwdownloads'] = wtwdownloads();	
?>		
