...
WTWJS.prototype.communitySearch = async function(zsearch) {
	...
	try {
		...
		WTW.getAsyncJSON('https://d3net.bixma.com/connect/sharesearch.php?search=' + zsearch + '&webtype=community', 
			function(zresponse) {
...
WTWJS.prototype.communitySearchReply = function(zresponse) {
	...
	try {
		...
		for (var i=0; i < zresponse.length; i++) {
			...
			ztempsearchresults += "<input type='button' id='wtw_bcommtempselect" + i + "' class='wtw-searchresultbutton' value='Download' onclick=\"WTW.downloadWeb('community', '" + btoa(zresponse[i].templatename) + "', '" + zcommunityid + "', '" + zcommunityid + "', 'community');\" " + zbuttonstyle + " />";
...
WTWJS.prototype.buildingSearch = async function(zsearch) {
	...
	try {
		...
		WTW.getAsyncJSON('https://d3net.bixma.com/connect/sharesearch.php?search=' + zsearch + '&webtype=building&buildingtype=' + zbuildingtype, 
			function(zresponse) {
...
WTWJS.prototype.thingSearch = async function(zsearch) {
	...
	try {
		...
		WTW.hide('wtw_downloadcomplete');
		zsearch = WTW.encode(zsearch);
		WTW.getAsyncJSON('https://d3net.bixma.com/connect/sharesearch.php?search=' + zsearch + '&webtype=thing', 
			function(zresponse) {
...
WTWJS.prototype.avatarSearch = async function(zsearch) {
	...
	try {
		...
		WTW.hide('wtw_downloadcomplete');
		zsearch = WTW.encode(zsearch);
		WTW.getAsyncJSON('https://d3net.bixma.com/connect/sharesearch.php?search=' + zsearch + '&webtype=avatar', 
			function(zresponse) {
...
WTWJS.prototype.pluginSearch = async function(zsearch) {
	...
	try {
		...
		WTW.hide('wtw_downloadcomplete');
		zsearch = WTW.encode(zsearch);
		WTW.getAsyncJSON('https://d3net.bixma.com/connect/sharesearch.php?search=' + zsearch + '&webtype=plugin', 
			function(zresponse) {
...
WTWJS.prototype.updateCols = function(zobj, zcols) {
	try {
		...
		WTW.log('core-scripts-prime-wtw_downloads.js-updateCols=' + ex.message);
	}
}
...
WTWJS.prototype.getDownloadsInfo = async function(zdownloads, zshow) {
	/* uses the local downloads to get the download info from d3net.bixma.com hub */
	try {
		...
		WTW.postAsyncJSON('https://d3net.bixma.com/connect/downloadsinfo.php', zrequest, 
			function(zresponse) {
				zresponse = JSON.parse(zresponse);
				/* note serror would contain errors */
